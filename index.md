...
\* In the current version, only **LaMP** over **IPv4** and **UDP** is supported, but we plan to implement other protocols in the future (LaMP over AMQP is work in progress).

# LaMP specifications

The full LaMP specifications can be found here: [LaMP specifications, revision 2.0](./LaMP/LaMP_specifications_rev2.0.pdf).

# LaTe user guide (v0.1.4-beta+)
...
- **-U** for an experimental unidirectional mode, in which the client sends a LaMP packet to the server, and the latter tries to compute the latency, based on its own timestamp and on the one embedded by the client inside the packet, without the need of generating a reply. It works only if there is an external way of keeping the devices’ clocks perfectly synchronized, with ms precision. As this is not an easy task, this mode shall remain experimental and it should never be preferred over the ping-like one.
...
| -A | Access category, string: **BK** or **BE** or **VI** or **VO** | Forces a certain EDCA MAC access category to be used. This option requires a pathed kernel to work (a warning will be printed about this), but it has been successfully used to perform latency measurements over 802.11p, using a patched version of OpenWrt, i.e. using either OpenC2X-Embedded (https://github.com/florianklingler/OpenC2X-embedded) or OpenWrt-V2X (https://github.com/francescoraves483/OpenWrt-V2X). | (not set, i.e. AC_BE) |
| -L | Latency type, char: **u**, **r**, **s** or **h** | Select latency type: user-to-user, KRT (Kernel Receive Timestamp), software kernel transmit and receive timestamps (only when supported by the NIC) or hardware timestamps (only when supported by the NIC). Additional types will probably be added in future versions of the program. | u |
...
| -F | - | Enable the LaMP follow-up mechanism (defined starting from *LaMP rev 2.0*). At the moment only the ping-like mode is supporting this. This mechanism will send an additional follow-up message after each server reply, containing an estimate of the server processing time, which is computed depending on the chosen latency type.  | (disabled) |
| -W | Filename, without extension | Write, for the current test only, the single packet latency measurement data to the specified CSV file. If the file already exists, data will be appended to the file, with a new header line. Warning! This option may negatively impact performance. | - |
...
| -L | Latency type, char: **u** or **r** | Select latency type: user-to-user or KRT (Kernel Receive Timestamp). Please note that the server supports this parameter only when in unidirectional mode. If a bidirectional INIT packet is received, the mode is completely ignored. Additional types will probably be added in future versions of the program. For the time being, no *s* or *h* types are supported in unidirectional mode. | u |
...
| -e | - | Bind to a non-wireless interface instead of looking for wireless interfaces. | (wireless) |
| -p | Port, integer, *> 0* and *< 65535* | Specifies the port to be used. Can be specified only if protocol is UDP. | 46000 |
| -0 | - | Force refusing follow-up mode, even when a client is requesting to use it. | (accepted) |
...
- There is currently no support for software kernel transmit and receive timestamps and hardware timestamps when in raw or unidirectional mode.
- The hardware timestamp feature is still experimental: it should work properly, and several tests have been performed on supported NICs, but more testing is still required.
...
**Software transmit and receive timestamps ('s')**: this mode is available only if supported by the NIC and by the driver. Both the sender and the receiver timestamps are obtained from the Linux kernel, as every request is sent and every reply is received. This feature relies on ancillary data and [CMSG](http://man7.org/linux/man-pages/man3/cmsg.3.html). *This latency type, as of now, is supported only for RTT measurements (i.e. in -B mode) and when non-raw sockets are used (i.e. no -r is specified)*

**Hardware transmit and receive timestamps ('h')**: this mode is available only if supported by the NIC and by the driver. Both the sender and the receiver timestamps are obtained from the network adapter, as every request is sent and every reply is received, so, no kernel time should be considered. This feature relies on ancillary data and [CMSG](http://man7.org/linux/man-pages/man3/cmsg.3.html). *This latency type, as of now, is supported only for RTT measurements (i.e. in -B mode) and when non-raw sockets are used (i.e. no -r is specified).*
